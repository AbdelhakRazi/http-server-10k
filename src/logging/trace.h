...
namespace trace {
#ifdef DEBUG
    #define TRACE_DEBUG(...) format_str(__FILE_NAME__, __LINE__, "[DEBUG] ", __VA_ARGS__)
#else
    #define TRACE_DEBUG(...) // No-op when DEBUG is not defined
#endif // DEBUG

#define TRACE_INFO(...) format_str(__FILE_NAME__, __LINE__,  "[INFO] ",  __VA_ARGS__)
#define TRACE_ERROR(...) format_str(__FILE_NAME__, __LINE__, "[ERROR] ", __VA_ARGS__)
